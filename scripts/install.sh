...
install_from_source() {
  ...
  if [ "$VERSION" = "latest" ]; then
    die "source build requires a specific version tag — use --version v1.2.3 (not 'latest')"
  else
    ...
    # public key to be imported. Hard-fails if gpg is absent — integrity
    ...
    if have gpg; then
      ...
      die "gpg not found — required to verify tag signature for $VERSION (install gnupg and re-run)"
    fi
  fi
